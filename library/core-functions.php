<?php
/* Welcome to lierphotography :)
This is the core lierphotography file where most of the
...
add_action('after_setup_theme','lierphotography_start', 16);

function lierphotography_start() {

    // launching operation cleanup
    add_action('init', 'lierphotography_head_cleanup');
    // remove WP version from RSS
    add_filter('the_generator', 'lierphotography_rss_version');
    // remove pesky injected css for recent comments widget
    add_filter( 'wp_head', 'lierphotography_remove_wp_widget_recent_comments_style', 1 );
    // clean up comment styles in the head
    add_action('wp_head', 'lierphotography_remove_recent_comments_style', 1);
    // clean up gallery output in wp
    add_filter('gallery_style', 'lierphotography_gallery_style');

    // enqueue base scripts and styles
    add_action('wp_enqueue_scripts', 'lierphotography_scripts_and_styles', 999);
    // ie conditional wrapper

    // launching this stuff after theme setup
    lierphotography_theme_support();

    // adding sidebars to Wordpress (these are created in functions.php)
    add_action( 'widgets_init', 'lierphotography_register_sidebars' );
    // adding the lierphotography search form (created in functions.php)
    add_filter( 'get_search_form', 'lierphotography_wpsearch' );

    // cleaning up random code around images
    add_filter('the_content', 'lierphotography_filter_ptags_on_images');
    // cleaning up excerpt
    add_filter('excerpt_more', 'lierphotography_excerpt_more');

} /* end lierphotography start */
...
function lierphotography_head_cleanup() {
	// category feeds
...
} /* end lierphotography head cleanup */

// remove WP version from RSS
function lierphotography_rss_version() { return ''; }

// remove injected CSS for recent comments widget
function lierphotography_remove_wp_widget_recent_comments_style() {
...
function lierphotography_remove_recent_comments_style() {
...
function lierphotography_gallery_style($css) {
...
function lierphotography_scripts_and_styles() {
...
function lierphotography_theme_support() {
...
} /* end lierphotography theme support */

/*********************
RELATED POSTS FUNCTION
*********************/

// Related Posts Function (call using lierphotography_related_posts(); )
function lierphotography_related_posts() {
	echo '<ul id="lierphotography-related-posts">';
	global $post;
	...
	if($tags) {
        ...
        if($related_posts) {
            ...
            <?php echo '<li class="no_related_post">' . __( 'No Related Posts Yet!', 'lierphotographytheme' ) . '</li>'; ?>
		<?php }
	}
	wp_reset_query();
	echo '</ul>';
} /* end lierphotography related posts function */
...
function lierphotography_page_navi($before = '', $after = '') {
	...
	if ($start_page >= 2 && $pages_to_show < $max_page) {
		$first_page_text = __( "First", 'lierphotographytheme' );
	...
	echo '</li>';
	if ($end_page < $max_page) {
		$last_page_text = __( "Last", 'lierphotographytheme' );
...
// Search Form
function lierphotography_wpsearch($form) {
	$form = '<form role="search" method="get" id="searchform" action="' . home_url( '/' ) . '" >
	<label class="screen-reader-text" for="s">' . __('Search for:', 'lierphotographytheme') . '</label>
	<input type="text" value="' . get_search_query() . '" name="s" id="s" placeholder="'.esc_attr__('Search the Site...','lierphotographytheme').'" />
...
function lierphotography_filter_ptags_on_images($content){
   return preg_replace('/<p>\s*(<a .*>)?\s*(<img .* \/>)\s*(<\/a>)?\s*<\/p>/iU', '\1\2\3', $content);
}

// This removes the annoying […] to a Read More link
function lierphotography_excerpt_more($more) {
	global $post;
	// edit here if you like
return '...  <a class="excerpt-read-more" href="'. get_permalink($post->ID) . '" title="'. __('Read', 'lierphotographytheme') . get_the_title($post->ID).'">'. __('Read more &raquo;', 'lierphotographytheme') .'</a>';
}
 ...
 */
function lierphotography_get_the_author_posts_link() {
	global $authordata;
